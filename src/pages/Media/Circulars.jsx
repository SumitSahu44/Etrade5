...
import { FileText, Calendar, ArrowRight, Bell, Download, AlertCircle } from 'lucide-react';

const Circulars = () => {
  const circulars = []; // No circulars at present
    ...
    <div className="min-h-screen bg-[#f1f5f9] font-sans relative overflow-hidden">
      ...
      <section className="bg-slate-950 pt-24 pb-48 px-6 relative">
        ...
        {/* <div className="max-w-4xl mx-auto relative z-10 text-center">
          ...
          >
            <Bell size={14} className="animate-bounce" /> Official Updates
          </motion.div>
          <h1 className="text-4xl md:text-6xl font-black text-white tracking-tighter uppercase mb-4">
            Official <span className="text-blue-500">Circulars</span>
          </h1>
          <p className="text-slate-400 font-medium max-w-lg mx-auto text-sm md:text-base">
            Stay informed with the latest regulatory and trade notifications.
          </p>
        </div> */}
      </section>

      {/* --- CIRCULARS LIST / FALLBACK --- */}
          ...
          {circulars.length > 0 ? (
            circulars.map((item, idx) => (
              ...
              >
                ...
                <button className="p-4 rounded-2xl bg-slate-900 text-white group-hover:bg-blue-600 transition-all">
                  <ArrowRight size={20} />
                </button>
              </motion.div>
            ))
            ...
            <motion.div
              ...
              className="bg-white/80 backdrop-blur-xl p-12 md:p-20 rounded-[3rem] border border-white shadow-2xl text-center"
            >
              ...
              {/* <p className="text-slate-500 font-medium max-w-sm mx-auto">
                All official notices and trade circulars will be published here as they become available.
              </p> */}
            </motion.div>
          )}
        </div>

        {/* --- ARCHIVE BUTTON (Hidden if no circulars) --- */}
        {circulars.length > 0 && (
