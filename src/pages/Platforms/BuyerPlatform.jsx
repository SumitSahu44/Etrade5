...
const BuyerPlatform = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const form = e.target;
    // Simple HTML5 FormData approach
    const formData = new FormData();
    formData.append('authorizedPersonId', authId);
    formData.append('buyerName', form.buyerName.value);
    formData.append('businessTitle', form.businessTitle.value);
    ...
    formData.append('textileItemsToBuy', form.textileItemsToBuy.value);
    formData.append('itemDescription', form.itemDescription.value);
    formData.append('requiredQuantity', form.requiredQuantity.value);
    formData.append('tentativeRate', form.tentativeRate.value);
    formData.append('tentativeBudget', form.tentativeBudget.value);
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-10 border-t border-slate-100">
                  ...
                  <div>
                    ...
                    <input type="text" name="buyerName" required className={inputStyle} />
                  </div>
                  <div>
                    <label className={labelStyle}>Title of the Business</label>
                    <input type="text" name="businessTitle" required className={inputStyle} />
                  </div>
                  <div className="md:col-span-2">
                    <label className={labelStyle}>Address of the Business</label>
                    <textarea name="businessAddress" required className={`${inputStyle} h-28 resize-none`} placeholder="Complete Office Address"></textarea>
                  </div>
                  <div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input type="text" name="chamberMembership" className={inputStyle} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-10 border-t border-slate-100">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <select name="textileItemsToBuy" required className={inputStyle}>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea name="itemDescription" className={`${inputStyle} h-28 resize-none`} placeholder="Specify Material, Grade, Loom Type, or Spare Specs..."></textarea>
                  </div>
                  <div>
                    <label className={labelStyle}>Required Quantity</label>
                    <input type="text" name="requiredQuantity" required className={inputStyle} />
                  </div>
                  <div>
                    <label className={labelStyle}>Tentative Rate for Buying</label>
                    <input type="text" name="tentativeRate" className={inputStyle} />
                  </div>
                  <div className="md:col-span-2">
                    <label className={labelStyle}>Tentative Budget for Buying</label>
                    <input type="text" name="tentativeBudget" className={inputStyle} />
                  </div>
                </div>
        ...
        {showPreview && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-900/60 backdrop-blur-xl z-[300] flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} className="bg-white rounded-[3rem] w-full max-w-2xl p-12 relative border border-white">
              ...
              <button type="button" onClick={() => setShowPreview(false)} className="w-full py-5 bg-blue-600 text-white font-black rounded-2xl uppercase  text-xs shadow-xl shadow-blue-200">
                Back to Edit
              </button>
            </motion.div>
          </motion.div>
